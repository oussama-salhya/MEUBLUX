...
import { createProductThunk, deleteProductThunk, editProductThunk, getAllProductsThunk, getFilteredProductsThunk, getSearchSuggestionsThunk, getSingleProductThunk, uploadImagesThunk } from "./productsThunk";
...
export const getSingleProduct = createAsyncThunk('products/getSingleProduct', getSingleProductThunk)
export const createProduct = createAsyncThunk('products/createProduct', createProductThunk)
export const editProduct = createAsyncThunk('products/editProduct', editProductThunk)
export const deleteProduct = createAsyncThunk('products/deleteProduct', deleteProductThunk)
export const getFilteredProducts = createAsyncThunk('/products/getFilteredProducts', getFilteredProductsThunk)
export const getSearchSuggestions = createAsyncThunk('/products/getSearchSuggestions', getSearchSuggestionsThunk)
const ProductsSlice = createSlice({
    ...
    reducers: {
        ...
        clearFilters: (state) => {
            return { ...state, ...initialFilters, search: state.search, price: state.maxPrice }
        },
        ...
        toggleFilter: (state, { payload: { name, value } }) => {
            ...
            if (Array.isArray(state[name])) {
                if (state[name].includes(value)) { // we will remove the filter because it exists
                    state[name] = state[name].filter(item => item !== value)
                }
                else { // we will then add filter
                    state[name].push(value)
                }
            }
        ...
        deleteImgFromSingleProductImages: (state, { payload: id }) => {
            const images = state.singleProduct.images.filter((item, index) => index !== id)
        ...
        setupFilters: (state, { payload }) => {
            ...
            if (state.maxPrice !== Number(state.price)) {
                ...
                state.categoriesOptions = state.categoriesOptions.map(category => ({ ...category, enabled: true }))
                state.companiesOptions = state.companiesOptions.map(company => ({ ...company, enabled: true }))
            }
            ...
            state.companies = state.companies.filter(company => payload.companies.includes(company))
            state.categories = state.categories.filter(category => payload.categories.includes(category))
        },
        changeSingleProductRating: (state, { payload: { numOfReviews, averageRating } }) => {
    ...
    extraReducers: {
        ...
        [getAllProducts.fulfilled]: (state, { payload }) => {
            ...
            state.categoriesOptions = payload.categories.map(category => ({ name: category, enabled: true }))
            state.companiesOptions = payload.companies.map(company => ({ name: company, enabled: true }));
...
export const { setFilter, incrementPage, setProgressPercentage, changeSingleProductRating, setupFilters, handleChange, handleSearchBar, hideMsg, clearMsgContent, displayError, hideError, openFilterMenu, closeFilterMenu, toggleFilter, clearFilters, clearSingleProductValues, openActionsButton, closeActionsButton, appendImgtoSingleProductImages, changeSingleProductValues, deleteImgFromSingleProductImages, showLoading, hideLoading } = ProductsSlice.actions
