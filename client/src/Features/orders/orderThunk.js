...
export const createPaymentIntentThunk = async (_, thunkAPI) => {
    ...
    try {
        const { data } = await customFetch.post('/orders/create-payment-intent', newCart)
        return data;
    } catch (error) {
...
export const createOrderThunk = async (paymentIntentId, thunkAPI) => {
    ...
    try {
        await customFetch.post('/orders/createOrder', { paymentIntentId, orderItems })
    } catch (error) {
        console.log(JSON.stringify(error));
    }
}
...
export const getFilteredOrdersThunk = async (_, thunkAPI) => {
    const { companies, categories, cities, page, month } = thunkAPI.getState().orders;
...
export const getTotalOrdersGroupByCategoriesThunk = async () => {
    try {
        const { data } = await customFetch.get('/orders/get-total-orders-by-category')
        return data
    } catch (error) {
        console.log(error);
    }
...
export const getSingleProductOrdersThunk = async (productId, date) => {
    ...
    try {
        const { data } = await customFetch.get(`/orders/get-product-orders?i=${productId}`)//&d=${date}
        return data
    } catch (error) {
        console.log(error);
    }
