...
import { changeSingleProductRating } from "../products/ProductsSlice";
...
export const deleteReviewThunk = async (reviewId, thunkAPI) => {
    ...
    try {
        const { data: { numOfReviews, averageRating } } = await customFetch.delete(url)
        thunkAPI.dispatch(changeSingleProductRating({ numOfReviews, averageRating }))
        return numOfReviews
    } catch (error) {
        console.log(error);
...
export const updateReviewThunk = async (_, thunkAPI) => {
    const { reviews: { idReview, comment, rating }, user: { user } } = thunkAPI.getState()
    const url = `/reviews/${idReview}`;
    try {
        const { data: { review, numOfReviews, averageRating } } = await customFetch.patch(url, { comment, rating })
        review.user = user;
        thunkAPI.dispatch(changeSingleProductRating({ numOfReviews, averageRating }))
        return review
    } catch (error) {
...
export const createReviewThunk = async (product, thunkAPI) => {
    ...
    try {
        const { data: { review, numOfReviews, averageRating } } = await customFetch.post('/reviews', { comment, rating, product: productId })
        const { user } = thunkAPI.getState().user
        review.user = user;
        thunkAPI.dispatch(changeSingleProductRating({ numOfReviews, averageRating }))
        return review
